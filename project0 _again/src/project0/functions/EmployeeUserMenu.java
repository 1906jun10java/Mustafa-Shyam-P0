...
public class EmployeeUserMenu {
	...
	public static void EmpUsr() {
		...
		while (!exit){
			...
			EmployeeLogin.empChk(id, password);// checks if Employee is int the system(Hashmap)
			if(EmployeeLogin.empChk(id, password) == true) {
				EmployeeSwitch.EmployeeMenu();// if so, then employee gets access to the Employee Menu
			}else {
				exit = true;
			}//OtherWise, they are sent back to the main menu
		}
